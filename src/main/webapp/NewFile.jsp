...
<body>
	...
	<div>
		<form action="MainServlet" method="GET">
			<input type="hidden" name="action" value="make_apartment_order">
			<div>
				<table border="1">
		...
		<%-- <p class=" text-right">
    Hello, ${userSe.getName()}

    <c:if test="${userSe.getRole()==0}">
     <input type="submit" value="LOG_OUT"
      class="btn btn-info btn-sm button" form="out">
     <form action="login.jsp" id="out"></form>

    </c:if>
    <c:if test="${userSe.getRole()!=0}">

     <input id="submit" value="return" type="submit"
      class="btn btn-info btn-sm button" form="return">
